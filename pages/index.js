...
const notify = (text) =>
  toast(
    ...
    {
      icon: <div className={clsx("h-4 w-4 rounded-sm")} style={{ backgroundColor: text }}></div>,
      ...
      <div className="p-6">
        <header className="flex items-center justify-between mb-7 cetner">
          <div className="flex flex-col items-start space-x-0 xl:flex-row xl:items-end xl:space-x-2">
            ...
            <h2 className="text-gray-400">Click to copy the HEX value ✨</h2>
          </div>
          <a href="https://squale.agency">
            <code className="px-2 py-1 text-sm text-gray-800 bg-gray-100 rounded-md">squale.agency</code>
          </a>
        </header>
        <main>
                  ...
                  {colors[idx].map((color, i) => {
                    return (
                      <m.div
                        ...
                        className={
                          "aspect-w-1 focus:ring-2 focus:outline-none rounded aspect-h-1 ease-in-out cursor-pointer w-full"
                        }
                      ></m.div>
                    )
                  })}
